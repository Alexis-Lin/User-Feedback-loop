...
**General 流程（一级单页 + 二级独立页）：**
- **一级「问题反馈」**（单页）：问题类大类多选 → 描述(≤200) + 录音 + 截图 + 邮箱(选填) → 提交（静默附带设备信息）。
- **二级「新动作申请」**（独立可复用页，一级点导航行 push 进入，返回回一级）：动作名称(必填) + 理由(选填) + 参考视频(选填)。**因是独立页，动作库等其它入口也能直接复用它。**
- 两条提交后都进 **轻量满意度**（😍🙂😕 选填，避免反馈池全是抱怨、失真）。
- **区域版本（账号/渠道不同，按账号或地区自动切换）**：**大陆版** 联系方式走手机号/微信 + 引导**加企业微信客服**（问题复杂 / 想实时回复时更快）；**国际版** 走邮箱。
- 子类不再作为 General 的必选步骤（大类 + 描述即可）；课中报错仍走精确子类。

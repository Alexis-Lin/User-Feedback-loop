...
- **一级「问题反馈」**：纯分类菜单（AI识别准确性 / 课程内容 / 设备与数据 / 体验与建议 / **其他** / 新增动作申请），**点选即进二级、一级零输入不劝退**。「其他」= 兜底项（不确定选哪个 / 想说的比较多），二级**不带子选项**，直接引导写/说。
- **二级（每个分类）**：该分类的子选项多选 + 补充说明(小号、按内容自适应高度) + 录音 + 截图(紧凑) + 联系方式 → 提交（静默附带设备信息）。**交互与「课中单动作反馈」一致。**
- **二级「新增动作申请」**：独立可复用专属表单（动作名称必填 + 理由 + 参考视频），动作库等其它入口也能直接进它。
- 提交后 **轻量满意度**（😍🙂😕 选填，避免反馈池全是抱怨、失真）。
- **区域版本（按账号/地区自动切换）**：**大陆版** 联系方式走手机号/微信/邮箱 + 引导**加企业微信客服**（问题复杂 / 想实时回复时更快）；**国际版** 走邮箱。
- **官方客服邮箱**：一级页底部常显 `cs@bodypark.fit`（mailto，不想走流程可直接联系）。
- 课中报错仍走精确子类，与本页共用后端。

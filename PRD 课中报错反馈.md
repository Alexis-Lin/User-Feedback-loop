...
- **一级「问题反馈」**：纯分类菜单（AI识别准确性 / 课程内容 / 连接数据与使用体验 / 其他问题反馈 / 新功能建议，共 **5 项**），**点选即进二级、一级零输入不劝退**。「连接数据与使用体验」= 设备与数据 + 卡顿/交互 合并；「其他问题反馈」「新功能建议」无子选项、直接写。**「新功能建议」只收功能建议、不收 bug，走产品需求池而非 Bug 看板（提交后文案也不同）。**
...
- **中英双语**：语言（中/英）与版本（大陆/国际）**正交**、各自独立切换（英文 UI 也能配大陆联系渠道，反之亦然）；一级菜单每项带**单色极简 icon**；整体标题/说明文案已精简。
- **语言/版本跟随系统（重要）**：真机中语言 = **手机系统语言**、版本 = **系统区域 / 账号**自动判定，**前端不做页面级切换**；原型里的两个开关仅供本 PRD 与演示对照，不落地为界面。
- **「补充说明」兼作"其他 / 兜底"入口**：子选项**不单列"其他"**；文本框下方提示「没有合适的？想补充或有其他情况，打字 / 语音都行」——把兜底与补充统一收进这一个可打字 / 可语音的框。
- 课中报错仍走精确子类，与本页共用后端。

**降低阻力两点优化：**
- **邮箱默认填充**：有邮箱的账号（国外/国际版必然有）→ 联系方式默认展示**账号邮箱**（"回复会发到这里"），用户零操作；点「换一个」才展开一个**可选备用邮箱**。大陆版同理可默认填手机/微信。
- **highlight 选填**：所有可填可不填项的「选填」改为**浅底 pill 徽章**（不再是小灰字），让用户一眼确认能跳过；配合"选完类型就能提交"，把最小必填降到「只选一个分类」。

### 9.x AI 计划结果吐槽（第 4 个入口）

- **入口**：AI 生成计划的**结果卡左下角** 👍点赞 / 👎吐槽（吐槽 = 报错）。
- **单步表单**（无一级跳转）：4 类 MECE 问题 + 分割线后「建议新功能」+ 补充说明。
  - 生成没完成 / 生成速度太慢（生成过程）
  - 动作选得不对（目标/部位/器械——选什么）
  - 编排不合适（顺序/组数/次数/负重——怎么排）
  - **建议新功能**（与上面问题类**互斥、二选一**；走**产品需求池**而非 Bug 看板）
  - 其他或补充说明（打字 / 语音）
- **「已反馈」状态 + 可再报**：提交后卡片下方显示「已反馈」；**再次点击可重开、再提交一次**（不硬锁）。
- **报错按钮状态**：默认(实心浅底 chip，比旧的淡灰描边更醒目、不用红) / 按下(缩放+触感) / 已反馈(绿色确认，仍可点) / 冷却(刚提交 <3s 再点仅轻提示、不重开) / 不可用(生成未完成、离线队列满等置灰)。详见 [`报错按钮设计.html`](<报错按钮设计.html>) 与 [`Flutter Code 报错按钮与状态.dart`](<Flutter Code 报错按钮与状态.dart>)。
- **归并规则（后端）**：同 `(targetId + user)` 的多次报错 → 合并为**一条工单的多次补充**，不新开卡，避免同一对象被同一用户刷出重复工单。
